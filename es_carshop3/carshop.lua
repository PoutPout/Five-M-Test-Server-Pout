...
RegisterNUICallback('buy_vehicle', function(veh, cb)
    TriggerServerEvent('es_carshop3:buyVehicle', veh.vehicle)

    cb('ok')
end)

RegisterNetEvent("es_carshop3:sendOwnedVehicles")
AddEventHandler('es_carshop3:sendOwnedVehicles', function(v)
	SendNUIMessage({
        type = "vehicles",
        enable = v
    })
...
RegisterNetEvent("es_carshop3:sendOwnedVehicle")
AddEventHandler('es_carshop3:sendOwnedVehicle', function(v)
	SendNUIMessage({
        type = "vehicle",
        enable = v
    })
...
RegisterNetEvent('es_carshop3:closeWindow')
AddEventHandler('es_carshop3:closeWindow', function()
	EnableGui(false)
end)
...
RegisterNetEvent('es_carshop3:createVehicle')
AddEventHandler('es_carshop3:createVehicle', function(v, options)
	...
	if playerPed and playerPed ~= -1 and isOwnedVehicleSpawned == false then
		...
		TriggerServerEvent('es_carshop3:newVehicleSpawned', NetworkGetNetworkIdFromEntity(veh))
...
local player_owned = {}
RegisterNetEvent('es_carshop3:removeVehiclesDeleting')
AddEventHandler('es_carshop3:removeVehiclesDeleting', function()
	if isOwnedVehicleSpawned then
		...
		if DecorGetInt(GetVehiclePedIsIn(GetPlayerPed(-1), false), "owner") then
			...
			if DecorGetInt(GetVehiclePedIsIn(GetPlayerPed(-1), false), "owner") == GetPlayerServerId(PlayerId()) then
				...
				TriggerServerEvent('es_carshop3:vehicleRemoved')
...
Citizen.CreateThread(function()
	for k,v in ipairs(carshops) do
		TriggerEvent('es_carshop3:createBlip', 356, v.x, v.y, v.z)
	end
end)
...
RegisterNetEvent("es_carshop3:createBlip")
AddEventHandler("es_carshop3:createBlip", function(type, x, y, z)
...
RegisterNetEvent('es_carshop3:setColour')
AddEventHandler('es_carshop3:setColour', function(r, g, b)
	SetVehicleCustomPrimaryColour(NetworkGetEntityFromNetworkId(spawnedVehicle),  r,  g,  b)
end)

RegisterNetEvent('es_carshop3:setColourSecondary')
AddEventHandler('es_carshop3:setColourSecondary', function(r, g, b)
