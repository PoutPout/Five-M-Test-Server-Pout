...
AddEventHandler("es:playerLoaded", function(source, target)
	...
	TriggerClientEvent("es_carshop2:sendOwnedVehicles", source, send)
end)
	...
	TriggerEvent("es_roleplay:getPlayerJob", function(job)
		if(job or (tonumber(user.permission_level) > 2))then
			...
			if(job.job == "police" or tonumber(user.permission_level) > 2)then
				...
				TriggerEvent('es_carshop2:getVehicleOwner', plate, function(owner, veh)
					local returns = ""
		...
		for i,v in pairs(players) do
			if(GetPlayerName(i))then
				TriggerEvent('es:getPlayerFromId', i, function(target)
					...
					TriggerClientEvent("es_carshop2:sendOwnedVehicles", i, send)
				end)
			end
		end
	end)
...
AddEventHandler('es_carshop2:getVehicleOwner', function(pl, cb)
	if(plates[pl])then
...
TriggerEvent('es:addCommand', 'rv', function(source, args, user)
	TriggerClientEvent('es_carshop2:removeVehiclesDeleting', source)
end)

RegisterServerEvent('es_carshop2:vehicleRemoved')
AddEventHandler('es_carshop2:vehicleRemoved', function()
...
RegisterServerEvent('es_carshop2:buyVehicle')
AddEventHandler('es_carshop2:buyVehicle', function(veh)
	if(spawned_vehicles[source] ~= nil)then
		TriggerClientEvent('es_carshop2:closeWindow', source)
		...
		TriggerEvent('es_roleplay:getPlayerJob', user.identifier, function(job)
			...
			if(vehicle_data[source])then
				for k,v in ipairs(vehicle_data[source]) do
					if(v.owner == user.identifier and veh == v.model)then
						...
						TriggerClientEvent('es_carshop2:removeVehicles', source)
						if(v.model == "police" or v.model == "mule" or v.model == "benson")then
							v.colour = "255,255,255"
						end
						TriggerClientEvent('es_carshop2:createVehicle', source, veh, { main_colour = stringsplit(v.colour, ","), secondary_colour = stringsplit(v.scolour, ","), plate = v.plate, wheels = v.wheels, windows = v.windows, platetype = v.platetype, exhausts = v.exhausts, grills = v.grills, spoiler = v.spoiler }  )
			...
			if(carshop_vehicles[veh])then
				...
				if(tonumber(user.money) >= price)then
					...
					TriggerClientEvent('es_carshop2:sendOwnedVehicle', source, veh)
...
RegisterServerEvent('es_carshop2:newVehicleSpawned')
AddEventHandler('es_carshop2:newVehicleSpawned', function(veh)
	if(spawned_vehicles[source])then
		if(spawned_vehicles[source] ~= veh)then
			TriggerClientEvent('es_carshop2:removeNetworkVehicle', -1, spawned_vehicles[source])
...
RegisterServerEvent('es_carshop2:vehicleCustom')
AddEventHandler('es_carshop2:vehicleCustom', function(model, data)
	if(true)then
		if(spawned_vehicles[source] ~= nil)then
...
function addVehicle(s, v)
	TriggerEvent('es:getPlayerFromId', s, function(user)
		...
		TriggerClientEvent('es_carshop2:removeVehicles', source)
		TriggerClientEvent('es_carshop2:createVehicle', source, v, { main_colour = stringsplit("0,0,0", ","), secondary_colour = stringsplit("0,0,0", ","), plate = plate, wheels = v.wheels, windows = v.windows, platetype = v.platetype, exhausts = v.exhausts, grills = v.grills, spoiler = v.spoiler })
